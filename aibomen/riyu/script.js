...
const state = {
  ...
  genre: "全部",
  query: "",
  view: localStorage.getItem("songlist-view-riyu") || "list"
};
...
function searchable(song) {
  return [song.title, song.artist, ...songGenres(song), song.note]
    .filter(Boolean)
    .join(" ")
    .toLowerCase();
}
...
function setView(view) {
  state.view = view;
  localStorage.setItem("songlist-view-riyu", view);
...
function drawRandom() {
  ...
  const meta = [randomSong.artist, ...songGenres(randomSong)].filter(Boolean).join(" · ");
  el.randomMeta.textContent = meta || "莉由_riyu 歌单";
...
async function init() {
  try {
    ...
    if (Array.isArray(data.items)) {
      state.all = data.items.filter(x => x && x.title);
    } else if (data.groups && typeof data.groups === "object") {
      state.all = [];
      for (const [genre, artists] of Object.entries(data.groups)) {
        for (const [artist, titles] of Object.entries(artists || {})) {
          for (const title of Array.isArray(titles) ? titles : []) {
            if (title) state.all.push({ title, artist, genre });
          }
        }
      }
    } else {
